//ADLIBEMU.C by Ken Silverman (http://advsys.net/ken)
/*
...
#define KEXTENDEDADLIB 0 //Ugly hacks! Set to 1 ONLY when linking to DIGISM!!!
...
// Changed on the advice of Ken from the above to the following
...
static int numspeakers, bytespersample;
...
static unsigned char adlibreg[256<<KEXTENDEDADLIB], ksl[8][16];
...
int rend = 0;
#define FIFOSIZ 256
static float *rptr[9<<KEXTENDEDADLIB], *nrptr[9<<KEXTENDEDADLIB];
static float rbuf[9<<KEXTENDEDADLIB][FIFOSIZ*2];
...
static inline void ftol (float f, int *a) { (*a) = (int)f; }
...
static int waveform[8] = {WAVPREC,WAVPREC>>1,WAVPREC,(WAVPREC*3)>>2,0,0,(WAVPREC*5)>>2,WAVPREC<<1};
static int wavemask[8] = {WAVPREC-1,WAVPREC-1,(WAVPREC>>1)-1,(WAVPREC>>1)-1,WAVPREC-1,((WAVPREC*3)>>2)-1,WAVPREC>>1,WAVPREC-1};
static int wavestart[8] = {0,WAVPREC>>1,0,WAVPREC>>2,0,0,0,WAVPREC>>3};
...
static int initfirstime = 0;
void adlibinit (int dasamplerate, int danumspeakers, int dabytespersample)
{
	int i, j, frn, oct;
	float f;
	...
	if (!initfirstime)
	{
		...
		for(j=6;j>=0;j--)
			for(i=0;i<16;i++)
			{
				oct = (int)ksl[j+1][i]-8; if (oct < 0) oct = 0;
...
static int fpuasm;
static float fakeadd = 8388608.0+128.0;
static _inline void clipit8 (float f, INT_PTR a)
{
	_asm
	{
		mov edi, a
...
static _inline void clipit8 (float f, INT_PTR a)
{
	*(unsigned char *)a = min(max((int)f,0),255);
}
...
static inline void clipit8 (float f, INT_PTR a)
{
	*(unsigned char *)a = min(max((int)f,0),255);
}

static inline void clipit16 (float f, INT_PTR a)
{
	*(signed short *)a = min(max((int)f,-32768),32767);
}
...
void adlibgetsample (void *sndptr, int numbytes)
{
	...
	for(ns=0;ns<numsamples;ns+=endsamples)
	{
		...
		if (numspeakers == 1)
		{
			if (bytespersample == 1)
			{
				for(i=endsamples-1;i>=0;i--)
					clipit8(nrptr[0][i]*nlvol[0],i+(INT_PTR)sndptr);
			}
			else
			{
				for(i=endsamples-1;i>=0;i--)
					clipit16(nrptr[0][i]*nlvol[0],i+i+(INT_PTR)sndptr);
			}
		}
		else
		{
			...
			if (bytespersample == 1)
			{
				for(i=(endsamples<<1)-1;i>=0;i--)
					clipit8(snd[i],i+((INT_PTR)sndptr));
			}
			else
			{
                for (i = (endsamples << 1) - 1; i >= 0; i--) {
		...
		sndptr = (void *)(((INT_PTR)sndptr)+(numspeakers*bytespersample*endsamples));
